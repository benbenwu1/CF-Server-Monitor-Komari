...
- [x] 独立 Session 管理 Tab 按需加载，展示脱敏设备元数据、当前/在线状态，支持当前令牌轮换和跨设备确认撤销；切换站点时会丢弃过期响应。
- [ ] 下一工作包为 TOTP 2FA；OAuth/OIDC 和其他 P1 工作包尚未开始。

Session 工作包为 48 项 Node 测试通过，生产构建和 Wrangler dry-run 通过；尚未部署。升级后旧的无 `sid` JWT 会失效，管理员需要重新登录一次。
